...
ggplot(plot_data, aes(x = value, y = immediate_cause, fill = run)) +
  geom_col(position = "dodge") +
  geom_text(
    data     = plot_data %>% filter(metric == "% Agreement"),
    aes(label = paste0(value, "%")),
...
ggplot(plot_data_state, aes(x = value, y = factor, fill = run)) +
  geom_col(position = "dodge") +
  geom_text(
...
ggplot(plot_data_grade, aes(x = factor, y = value, fill = run)) +
  geom_col(position = "dodge") +
  geom_text(
...
bind_rows(
  cause_agreement_summary  %>% mutate(risk_type = "Immediate Cause"),
  obj$summary              %>% mutate(risk_type = "Objective Risks"),
  subj$summary             %>% mutate(risk_type = "Subjective Risks"),
  soc$summary              %>% mutate(risk_type = "Social Risks"),
  state_agreement_summary  %>% mutate(risk_type = "State / Region"),
  grade_agreement_summary  %>% mutate(risk_type = "YDS Grade", factor = as.character(factor))
) %>%
  mutate(risk_type = fct_reorder(risk_type, pct_agreement, .fun = median)) %>%
  ggplot(aes(x = pct_agreement, y = risk_type)) +
