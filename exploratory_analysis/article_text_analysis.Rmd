...
*Metadata for climb year is inconsistent before 2012*

```{r visualize distribution over time, fig.width=10.5, echo=FALSE}

p <- articles_tagged %>%
    group_by(publication_year, article_type) %>%
    summarise(articles = n(), .groups = "drop") %>%
    mutate(date = publication_year, date_type = "Publication Year") %>%
    bind_rows(
        articles_tagged %>%
            group_by(climb_year, article_type) %>%
            summarise(articles = n(), .groups = "drop") %>%
            mutate(date = climb_year, date_type = "Climb Year")
    ) %>%
...
```{r summary numbers, fig.width=10.5, echo=FALSE}
...
p <- articles_tagged %>%
    ...
    summarise(articles = n(), .groups = "drop") %>%
    ggplot(aes(y = publication_decade, x = articles, fill = article_type)) +
    geom_col() +
    facet_wrap(~article_type, nrow = 1) +
    scale_fill_manual(values = report_colors) +
    labs(
        title = "Accident Reports by Publication Decade",
        x = "Articles",
        y = "Publication Decade"
    ) +
    theme_minimal() +
